...
// void mha_WAR_data_mover(mfdata_v_t *cache, s_mfdata_v_t &output, s_fdata_v_t &input, const int CURR_LAYER, const int POS){
...
// 	const int head_offset = MODEL_SEQUENCE_LEN * vec_per_head;
// 	const int pos_offset = POS * vec_per_head;
// 	const size_t ratio = MAX_FL_ELEM/SM_FL_ELEM;
...
// 		for (int j = 0; j < ratio; j++) {
// 			#pragma HLS PIPELINE II=1
// 			fdata_v_t tmp = input.read();
// 			for (int k = 0; k < SM_FL_ELEM; k++) {
// 				mtmp[j * SM_FL_ELEM + k] = tmp[k];
// 			}
// 		}
// 		cache_array[i] = mtmp;
// 	}
