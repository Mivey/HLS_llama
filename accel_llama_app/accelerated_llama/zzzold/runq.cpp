...
// the only helper functions that should be left will be for the coin/RNG
// and token processing and printing. 
...
There will be 4 modes for calling forward: 
		1) load the rms weights & prefill (assuming prompt_tokens > 1) f.set_rms_flag(true) and f.enable_prefill()
		2) load rms weights & decode (assuming prompt_tokens == 1) f.set_rms_flag(false) and f.enable_decode()
		3) prefill (prompt_tokens >= 2) f.set_rms_flag(false) and f.enable_prefill()
		4) decode f.set_rms_flag(false) and f.enable_decode()

There's probably no need to set the flags more than once, so I imagine some kind of for loop/while loop unroll to set and run forward once before going into the second loop 
when we get to that stage, but also I imagine being able to start the kernel, and then do whatever is needed for the next run, so part of that could be setting the flags for the next 
itteration. Speaking of, the Idea is to have two functions, a start/run (f.startForward();) and an end state where I wait (f.endforward()). Once the kernel starts, 
the idea is to print the previous token, get the next number from the rng for coin, and possibly set flags before waiting for the next token. 
I'd get the next token value from f.endForward() and use that to start the next iterration. or ignore it during prefill and use the next defined token.
*/
void newgen(Transformer *transformer, Tokenizer *tokenizer, Sampler* sampler, char* prompt, int steps, ForwardBlock &f){
	char *empty_prompt = "";
...
//         float* logits = f.runForward(token, pos);// forward(transformer, token, pos);
