...
=====================================================================================================*/
		...
		void startForward(const int token, const int pos, const float coin){
			...
			// ==AI instructions==
			/*
			No need for memcpy anymore, it's built into kernel. 
			I write the curr_token_i register here.
			*/
		...
		int endForward(){
			...
			// ==AI instructions==
			/*
			instead of syncing token_bo, I read curr_token_o register
			actually I don't think I need the token_bo object; it will be replaced with a 
			bigger dequantized embedding table			
			*/
	...
	private:
		...
		xrt::bo token_bo;
		//There are some xrt::bo that are missing, like the dequantized embeddiing table. I think that's the only one.
		float* token_map_f;
		int* token_map_i;
		char* token_map_c;
		...
		// ==AI instructions==
		// might be a good idea to link everything to the mha_forward.h file instead of separate points here.
		...
		void embedded_token_init(/* prob use xrt::bo to get data*/){ // ==AI instructions==
		//alternatively this could be apart of the weights_init sequence...
			/* The code from the test bench that works */
			/*
	...
	*/
	// This is the 93.75Mb of data associated with the *tokens m_axi port; it will read the 768 elements to the internal_tokens 
		}

		void run_init(){			// ==AI instructions==
		// convert this to be  xrt::ip based on the provided header file
