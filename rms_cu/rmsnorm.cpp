...
void rmsnorm_kernel(fdata_v_t *output, fdata_v_t *tokens, fdata_v_t *diff, fdata_v_t *weights, const int CURR_LAYER){
	...
	#pragma HLS INTERFACE mode=m_axi port=tokens						bundle=rms_tok	depth=RMS_DEPTH		offset=slave max_read_burst_length=64 max_write_burst_length=64
	#pragma HLS INTERFACE mode=m_axi port=output						bundle=rms_out_w	depth=RMS_DEPTH		offset=slave max_read_burst_length=64 max_write_burst_length=64
	#pragma HLS INTERFACE mode=m_axi port=weights						bundle=rms_out_w 	depth=RMS_DEPTH		offset=slave max_read_burst_length=64
	#pragma HLS INTERFACE mode=m_axi port=diff							bundle=rms_out_w 	depth=RMS_DEPTH		offset=slave max_read_burst_length=64
	...
	// #pragma HLS STREAM variable=s_rescon depth=(MODEL_ELEMENTS / SM_FL_ELEM)
	#pragma HLS STREAM variable=s_tokens_out depth=(MODEL_ELEMENTS / SM_FL_ELEM)
	// #pragma HLS STREAM variable=s_rescon_out depth=(MODEL_ELEMENTS / SM_FL_ELEM)
	...
	// #pragma HLS BIND_STORAGE variable=s_rescon type=fifo impl=bram
	#pragma HLS BIND_STORAGE variable=s_tokens_out type=fifo impl=bram
	// #pragma HLS BIND_STORAGE variable=s_rescon_out type=fifo impl=bram
	#pragma HLS BIND_STORAGE variable=s_weights type=fifo impl=bram
	
	// tok_load_input(s_tokens, tokens);
	mm2s_input_data(s_tokens, tokens, MODEL_ELEMENTS/SM_FL_ELEM);
	...
	s2mm_output_data(tokens, s_diff_out, MODEL_ELEMENTS / SM_FL_ELEM, 0);
