...
template<typename T, typename S>
void parse_results (const std::vector<T> &gold, const std::vector<T> &obs, double rel_tol = 0.03, double abs_tol = 1e-4, bool ignore_padding = true){
    int pass = 0;
    int fail = 0;
    int print_limit = 900; // Prevent console spam
    ...
    for (size_t i = 0; i < gold.size(); i++) {
        for (size_t j = 0; j < gold[i].size(); j++) {
            ...
            // THE FIX: Skip the element completely if it is a padded zero.
            // This prevents total_elements from inflating and keeps noise out of the math.
            if (ignore_padding && a == 0.0) {
                continue; 
            }
    ...
    std::cout << "==================================" << std::endl;
    std::cout << "VALID ELEMENTS EVALUATED: " << total_elements << std::endl;
    std::cout << "PASSED: " << pass << std::endl;
    std::cout << "FAILED: " << fail << std::endl;
    std::cout << "----------------------------------" << std::endl;
    std::cout << "MEAN SQUARED ERROR: " << mse << std::endl;
